...
class ChatClient:
    def __init__(self) -> None:
        self.deployment_name = os.environ["AZURE_OPENAI_COMPLETION_MODEL"]
        ...
        try:
            async for part in response_stream:
                ...
                if finish_reason == "tool_calls" and is_collecting_function_args:
                    ...
                    print(
                        f"function_name: {function_name} function_arguments: {function_arguments}"
                    )
                    ...
                    for connection_name, session_tools in mcp_tools.items():
                        if any(
                            tool.get("name") == function_name for tool in session_tools
                        ):
    ...
    async def generate_response(self, human_input, tools, temperature=0):
        ...
        while True:
            ...
            try:
                ...
                async for token in self._stream_and_process(
...
async def on_message(message: cl.Message):
    ...
    async for text in client.generate_response(
        human_input=message.content, tools=tools
    ):
